...
import (
	"linkedInLearning/tempService10/models"
	"testing"
)
...
func TestCityQuery(t *testing.T) {
	tests := []struct {
		...
		{testName: "negative month query", beach: true, ski: false, month: -1, name: "", wantErr: "month must be in the range [1,12]; got -1"},
		{testName: "large month query", beach: true, ski: false, month: 100, name: "", wantErr: "month must be in the range [1,12]; got 100"},
		{testName: "zero month query", beach: true, ski: false, month: 0, name: "", wantErr: "month must be in the range [1,12]; got 0"},
